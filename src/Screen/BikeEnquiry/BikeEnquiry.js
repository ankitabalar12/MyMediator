...
import { bikeenquiryapi } from '../../../APICall'


const BikeEnquiry = ({route}) => {
    ...
    useEffect(() => {
        const loginallData = async () => {
          ...
          console.log('finaluserdata ------bbbbb-----------',finaluserdata)
          setID(finaluserdata.id);
        }
        loginallData()
      }, [])
      const bike_id = route?.params;
      console.log('bike_id------------', bike_id)
    const Bikeenquiry = async () => {
        ...
        const data = {
         ...
         console.log('data -------------',data)
          const res = await bikeenquiryapi(global.url + 'enquiry ', data);
          console.log('res ><>><', res)
